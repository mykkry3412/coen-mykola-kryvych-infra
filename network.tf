locals {
  ...
  vpc_name      = "coen-vpc-test"
  subnet_region = "europe-west3"
  subnet_range  = "10.30.0.0/16"
}

resource "google_compute_network" "coen-vpc-test" {
...
resource "google_compute_subnetwork" "backstage_subnets" {
  depends_on = [
    google_compute_network.coen-vpc-test
  ]
  ...
  network       = google_compute_network.coen-vpc-test.id
...
resource "google_compute_global_address" "coen-vpc-test" {
...
resource "google_service_networking_connection" "coen-vpc-test" {
  network                 = google_compute_network.coen-vpc-test.id
  service                 = "servicenetworking.googleapis.com"
  reserved_peering_ranges = [google_compute_global_address.coen-vpc-test.name]
}
